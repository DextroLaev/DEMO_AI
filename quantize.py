...
def quantize(x: torch.Tensor,
             ...
             zp: torch.Tensor,
             qmin: torch.Tensor,
             qmax: torch.Tensor) -> torch.Tensor:
...
class ActFakeQuant(nn.Module):
    ...
    def __init__(self, n_bits=8, unsigned=True):
        ...
        self.frozen = False
        self.register_buffer("qmin", torch.tensor(0.))
        self.register_buffer("qmax", torch.tensor(0.))
    ...
    def freeze(self):
        ...
        self.zp.copy_(zp)
        self.qmin.copy_(torch.tensor(float(qmin)))
        self.qmax.copy_(torch.tensor(float(qmax)))
        self.frozen = True
...
@torch.no_grad()
def export_int8_model(model, weight_bits=8):
    export = {}
    ...
    for name, m in model.named_modules():
        if isinstance(m, (QuantConv2d, QuantLinear)):
            # real int weights
            w_int = quantize(
                m.weight, m.w_scale, m.w_zp, m.qmin, m.qmax
            ).to(torch.int8)

            export[name] = {
                "weight_int": w_int.cpu(),          # INT8 (1 byte)
                "scale": m.w_scale.cpu(),           # FP32 scalar
                "zp": m.w_zp.cpu(),                 # FP32 scalar
...
def fold_batchnorms(model):
    def _fold_bn_recursive(module):
        ...
        for i in range(len(children)):
            ...
            # Recurse first
            _fold_bn_recursive(child)
            
            # Check if current child is BatchNorm and previous is Conv-like
            if isinstance(child, nn.BatchNorm2d) and i > 0:
                prev_name, prev_child = children[i-1]
                
                if isinstance(prev_child, (nn.Conv2d, QuantConv2d)):
                    conv = prev_child
                    bn = child
                    ...
                    w_fused = w * (gamma / denom).view(-1, 1, 1, 1)
